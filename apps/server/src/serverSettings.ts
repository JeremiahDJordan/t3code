...
const PersistedOptionalProviderSettings = Schema.Struct({
  providers: Schema.optionalKey(
    Schema.Struct({
      ...
      bob: Schema.optionalKey(Schema.Struct({ enabled: Schema.optionalKey(Schema.Boolean) })),
...
function restoreUsedProviders(
  ...
  return {
    ...
    providers: {
      ...
      bob: {
        ...settings.providers.bob,
        enabled: persisted.providers?.bob?.enabled ?? usedProviders.has("bob"),
      },
      opencode: {
...
const PERSISTED_SERVER_SETTINGS_DEFAULTS = {
  ...
  providers: {
    ...
    bob: { ...DEFAULT_SERVER_SETTINGS.providers.bob, enabled: undefined },
